...
exports.CustomizationOptionRepository = void 0;
...
class CustomizationOptionRepository {
    async findAllBySlotId(slotId) {
        ...
        `;
        const res = await postgres_1.pgPool.query(query, [slotId]);
        return res.rows || [];
    }
    async findBySlotAndProduct(slotId, productId) {
        const query = `
            SELECT 
                ${OPTION_FIELDS},
                p.name AS "name"
        ...
        `;
        const res = await postgres_1.pgPool.query(query, [slotId, productId]);
        if (!res.rows[0]) {
            return null;
        }
        return res.rows[0];
    }
    async create(slotId, data) {
        const { productId, priceDelta, isDefault, displayOrder } = data;
        const query = `
            INSERT INTO customization_slot_option (customization_slot_id, product_id, price_delta, is_default, display_order)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING ${RETURN_FIELDS}
        `;
        const res = await postgres_1.pgPool.query(query, [slotId, productId, priceDelta, isDefault, displayOrder]);
    ...
    async update(slotId, data) {
        ...
        if (data.productId === undefined)
            return null; // Cannot update nothing
        ...
        if (fields.length === 1)
            return this.findBySlotAndProduct(slotId, data.productId);
        values.push(slotId);
        const query = `
...
exports.CustomizationOptionRepository = CustomizationOptionRepository;
